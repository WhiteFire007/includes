...
ConVar bhpDistance, bhpMaxAttempts, bhpCoolTime, bhpShowDamage, bhpShowDistance, bhpShowKills,
	bhpAvgDistance, bhpRankType, bhpListMax, bhpListType, CountsMax;

float fDistance, fCoolTime, fPouncePos[MAXPLAYERS+1][3];
int iMaxAttempts, iRankType, iListMax, iListType, iPounces[MAXPLAYERS+1], iPenalty[MAXPLAYERS+1], iCounterCommand[MAXPLAYERS+1],
	iListPage[MAXPLAYERS+1] = 1, iCountsMax;

Handle hPenaltyTime[MAXPLAYERS+1] = null;
bool bShowDamage, bShowDistance, bShowKills, bAvgDistance, bShowPenalty[MAXPLAYERS+1];
...
{
	name = "Brutal Hunter Pounce with explosion",
	author = "cravenge <3",
...
public void OnPluginStart()
{
	...
	CountsMax = CreateConVar("brutal_hunter_pounce_reset_max", "5", "Maximum To Reset Bhp", FCVAR_NOTIFY|FCVAR_SPONLY, true, 3.0, true, 10.0);
	...
	CountsMax.AddChangeHook(OnBHPCVarsChanged);
...
public Action TogglePenalty(int client, int args)
{
	...
	CPrintToChat(client, "{orange}[{lightgreen}R★G{orange}]{default} Penalty Notification: {olive}%s", (bShowPenalty[client]) ? "On" : "Off");
	return Plugin_Handled;
}
...
public Action ResetBHP(int client, int args)
{
	...
	{
		CPrintToChat(client, "{orange}[{lightgreen}R★G{orange}]{default} Invalid!");
		return Plugin_Handled;
	}
	
	if (iCounterCommand[client] < iCountsMax)
	{
		iCounterCommand[client]++;
		CPrintToChat(client, "{orange}[{lightgreen}R★G{orange}]{default}Counts Reset {orange}({olive}%i-/-%i{orange})", iCounterCommand[client], iCountsMax);
		iPenalty[client] = 0;
		iPounces[client] = 0;
	}
	else
	{
	    CPrintToChat(client, "{orange}[{lightgreen}R★G{orange}]{default}Limit Reached");
	}
...
public void OnAbilityUse(Event event, const char[] name, bool dontBroadcast)
{
	...
	if (StrEqual(sAbility, "ability_lunge", false))
	{
		...
		{
			CreateTimer(0.2, MakeFartherPounce, user);
		}
	}
}
...
public void OnLungePounce(Event event, const char[] name, bool dontBroadcast)
{
	...
	{
		CPrintToChat(pouncer, "{orange}[{lightgreen}R★G{orange}]{olive} Failed! ({orange}%.1f{olive} /`{orange}%.1f\x05)", GetVectorDistance(fPouncePos[pouncer], fLandedPos), fDistance);
		return;
	}
	
	if (iPounces[pouncer] > iMaxAttempts)
	{
		CPrintToChat(pouncer, "{orange}[{lightgreen}R★G{orange}]{olive} Maximum Limits Reached!%s", (GetUserAdmin(pouncer) == INVALID_ADMIN_ID) ? "" : " Type {olive}!bhp_reset{default} To Reset Your BHP!");
		return;
	}
	...
	if (iPenalty[pouncer] > 0)
	{
		...
		{
			CPrintToChat(pouncer, "{orange}[{lightgreen}R★G{orange}]{olive} Please Wait!");
		}
		return;
	}
	...
	SetEntProp(pouncer, Prop_Send, "m_iHealth", 1, true);
	IgniteEntity(pouncer, 2.0);
	...
	iPounces[pouncer] += 1;
	CPrintToChat(pouncer, "{orange}[{lightgreen}R★G{orange}]{default} Attempts: {orange}%d{default} / {orange}%i", iPounces[pouncer], iMaxAttempts + 1);
	
	if (bShowDamage && bShowDistance)
	{
		CPrintToChatAll("{orange}[{lightgreen}R★G{orange}] {olive}%N{default} Brutally Pounced {olive}%N{default}! {orange}[%.2f Units / %i Dmg]", pouncer, pounced, GetVectorDistance(fPouncePos[pouncer], fLandedPos), iDmg);
	}
	else if (bShowDamage)
	{
		CPrintToChatAll("{orange}[{lightgreen}R★G{orange}] {olive}%N{default} Brutally Pounced {olive}%N! {orange}[%i Dmg]", pouncer, pounced, iDmg);
	}
	else if (bShowDistance)
	{
		CPrintToChatAll("{orange}[{lightgreen}R★G{orange}] {olive}%N{default} Brutally Pounced {olive}%N! {orange}[%.2f Units]", pouncer, pounced, GetVectorDistance(fPouncePos[pouncer], fLandedPos));
	}
	else
	{
		CPrintToChatAll("{orange}[{lightgreen}R★G{orange}] {olive}%N{default} Brutally Pounced {olive}%N{default}!", pouncer, pounced);
	}
...
public Action ShowNextBP(Handle timer, any client)
{
	...
	{
		CPrintToChat(client, "{orange}[{lightgreen}R★G{orange}]{default} You Can Brutal Pounce Again!");
...
void BHPStats(int client, int iKill = 0, int iDamage = 0, int iDistance = 0, bool bUpdate = false, int iPage = 1)
{
	...
	if (!bUpdate)
	{
		...
		{
			CPrintToChat(client, "{orange}[{lightgreen}R★G{orange}]{default} No Saved Data!");
			delete kvStats;
